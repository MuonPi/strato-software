...
static std::unique_ptr<SPI::spiDevice> spi_dev{nullptr};
...
void hal_pin_rst(u1_t val)
{
    ...
    if (val == 0 || val == 1)
    { // drive pin
        pinMode(plmic_pins->rst, OUTPUT);       // NKRG changed order of pinmode and digitalwrite
...
static void hal_spi_init()
{
    ...
    std::uint8_t bits = 8;
    spi_dev = std::make_unique<SPI::spiDevice>();
    if (!spi_dev->init(devAddress,speed,mode,bits)){
        throw std::runtime_error("Failed to initialise SPI device.");
    }
}
...
static void hal_spi_trx(u1_t cmd, u1_t *buf, size_t len, bit_t is_read)
{
    if (len+1 >= MAX_FRAME_LEN){
        throw std::runtime_error("Tried to transfer too large frame length.");
    }
    std::string data{};
    if (spi_dev==nullptr){
        throw std::runtime_error("Tried to use spi_device without init.");
    }
    if (is_read){
        data = spi_dev->read(cmd, len);
        ...
        for (size_t i{0}; i < len; i++){
            data += static_cast<char>(buf[i]);
        }
        auto result = spi_dev->write(cmd, data);
        if (!result){
            throw std::runtime_error("Failed writing buf to spi_device.");
        }
    }
