...
class SEN0321:
	...
	def save_ozon():
		try:
			write_file(filename("ozon_raw", "csv", 5), "raw", SEN0321.read_ozon())
		except:
...
class BME280:
	...
	def save_pres():
		try:
			write_file(filename("pres_raw", "csv", 5), "raw", BME280.read_pres())
		except:
			print('BME280_pres_save_fail')
	def save_temp():
		try:
			write_file(filename("temp_raw", "csv", 5), "raw", BME280.read_temp())
		except:
			print('BME280_temp_save_fail')
	def save_humi():
		try:
			write_file(filename("humi_raw", "csv", 5), "raw", BME280.read_humi())
		except:
...
class QMC5883L:
	...
	def save_magn():
		try:
			write_file(filename("magn_raw", "csv", 5), "raw", QMC5883L.read_magn(), 3)
		except:
...
class VEML6075:
	...
	def save_uvse():
		try:
			write_file(filename("uvse_raw", "csv", 5), "raw", VEML6075.read_uvse())
		except:
			print('VEML_uvse_save_fail')
...
class MuonPi:
	def read_mcor():
		try:
			return read_logfile(muon_logfile(), "geoHash")
		except:
			print('geoHash_read_fail')
			return 'fail'
	def save_mcor():
		try:
			write_file(filename("mcor_raw", "csv", 5), "raw", MuonPi.read_mcor())
		except:
			print('geoHash_save_fail')
	def read_mhei():
		try:
			return read_logfile(muon_logfile(), "geoHeightMSL")
		except:
			print('geoHeightMSL_read_fail')
			return 'fail'
	def save_mhei():
		try:
			write_file(filename("mhei_raw", "csv", 5), "raw", MuonPi.read_mhei())
		except:
