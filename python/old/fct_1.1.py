...
def transmit(hdl, trm, dev):           # handle, zu sendender String, Geraet: s/l
	trm = dev + ';' + trm + '#'
...
def read_gps(hdl, tmo):        # hdl=handle, tmo=timeout   # timeout muss noch hinzugefügt werden  !!!!!!!!!!!!!
...
def maps_str(cor):      # cor=koordinaten mit , getrennt
	out = 'https://www.google.de/maps?q=' + str(cor)
	return out
