...
class VEML6075:
	...
	def save_uvse(dat):
		try:
			write_file(filename("uvse_raw", "csv", 5), "raw", dat)
		except:
...
class SIM7000E:
	def init(prt = 'ttyUSB3'):
		try:
			global sim
			sim = serial.Serial('/dev/' + prt, 115200)
			time.sleep(.1)
			sim.reset_input_buffer()
			#sim.write('AT\r'.encode())
			#SIM7000E.answer()
			sim.write('AT+CGNSHOT\r'.encode())	# hotstart einschalten
			SIM7000E.answer()
			#### fehlt ####################
	...
	def test():
		try:
			sim.write('AT\r'.encode())			# prüfen ob ansprechbar
	...
	def activate_gnss():
		try:
			sim.write('AT+CGNSHOT\r'.encode())	# hotstart einschalten
			SIM7000E.answer()
			sim.write('AT+CGNSPWR=1\r'.encode())	# GNSS einschalten
			SIM7000E.answer()
		except:
	...
	def deactivate_gnss():
		try:
			sim.write('AT+CGNSPWR=0\r'.encode())	# GNSS ausschalten
			SIM7000E.answer()
		except:
