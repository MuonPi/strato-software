...
class LoRa:
	...
	def uplinkSequenceNo():
		try:
			fil = open(os.path.dirname(__file__) + "/../raw/uplinkSequenceNo.txt", 'r')
			usn = int(fil.readlines()[0])						# usn = uplinkSequenceNo
			fil.close()
			usn += 1
			#print(usn)
			fil = open(os.path.dirname(__file__) + "/../raw/uplinkSequenceNo.txt", 'w')
			fil.write(str(usn))
			fil.close()
			return usn
		except:
...
def write_file(nam, dir, val):#, num = 1):		# name, value, number
	try:
		pat	= os.path.dirname(__file__) + "/../" + dir + "/" + nam	# path
		fil = open(pat, 'a')
