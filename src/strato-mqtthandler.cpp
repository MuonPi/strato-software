...
{
    reinterpret_cast<StratoMqttHandler *>(object)->callback_connected(result);
}

void wrapper_callback_disconnected(mosquitto * /*mqtt*/, void *object, int result)
{
    reinterpret_cast<StratoMqttHandler *>(object)->callback_disconnected(result);
}

void wrapper_callback_message(mosquitto * /*mqtt*/, void *object, const mosquitto_message *message)
{
    reinterpret_cast<StratoMqttHandler *>(object)->callback_message(message);
}

void StratoMqttHandler::callback_connected(int result)
{
    if (result == 1)
    {
...
void StratoMqttHandler::callback_disconnected(int result)
{
    if (result != 0)
    {
        if (connected())
...
void StratoMqttHandler::callback_message(const mosquitto_message *message)
{
    emit receivedMessage(message->topic, reinterpret_cast<char *>(message->payload));
}

void StratoMqttHandler::set_status(Status status)
{
    qDebug() << "set_status" << static_cast<int>(status);
    m_status = status;
}

bool StratoMqttHandler::isInhibited()
{
    ...
    : QObject(nullptr), m_station_id{station_id.toStdString()}, m_host{host.toStdString()}, m_port{port}, m_verbose{verbosity}
{
    ...
    if (!connect(this, &StratoMqttHandler::connection_status, this, &StratoMqttHandler::set_status))
    {
        qDebug() << "failed connecting StratoMqttHandler::set_status";
    }
    if (!connect(this, &StratoMqttHandler::mqttConnect, this, &StratoMqttHandler::onMqttConnect))
    {
        qDebug() << "failed connecting StratoMqttHandler::mqttConnect";
    }
    if (!connect(this, &StratoMqttHandler::mqttDisconnect, this, &StratoMqttHandler::onMqttDisconnect))
    {
        qDebug() << "failed connecting StratoMqttHandler::mqttDisconnect";
    }
}

StratoMqttHandler::~StratoMqttHandler()
{
    mqttDisconnect();
    cleanup();
}

void StratoMqttHandler::start(const QString &username, const QString &password)
{
...
void StratoMqttHandler::onMqttConnect()
{
    if (connected())
    {
        return;
...
void StratoMqttHandler::onMqttDisconnect()
{
    if (m_mqtt != nullptr)
    {
        if (m_status == Status::Connected)
...
auto StratoMqttHandler::connected() -> bool
{
    return (m_mqtt != nullptr) && (m_status == Status::Connected);
}

void StratoMqttHandler::initialise(const std::string &client_id)
{
    if (m_mqtt != nullptr)
    {
        cleanup();
...
void StratoMqttHandler::cleanup()
{
    if (connected())
    {
        mqttDisconnect();
...
void StratoMqttHandler::subscribe(const QString &topic)
{
    if (!connected())
    {
        return;
...
void StratoMqttHandler::unsubscribe(const QString &topic)
{
    if (!connected())
    {
        return;
...
auto StratoMqttHandler::publish_(const std::string &topic, const std::string &content) -> bool
{
    if (!connected())
    {
        return false;
...
void StratoMqttHandler::requestConnectionStatus()
{
